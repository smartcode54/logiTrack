...
export const DepartureStatus = ({ confirmedTime, currentAddress }: DepartureStatusProps) => {
  if (confirmedTime) {
    return (
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 space-y-4 animate-fadeIn">
        <div className="bg-gray-900 rounded-xl p-4 text-white space-y-2">
          <div className="flex items-center justify-between border-b border-white/20 pb-2">
            <h3 className="text-karabao-light font-black text-sm uppercase">ออกเดินทาง</h3>
  ...
  return (
    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center text-center space-y-4 animate-fadeIn">
      ...
      <div>
        <h3 className="font-black text-gray-800 text-sm uppercase">เตรียมออกเดินทาง</h3>
        <p className="text-[10px] text-gray-400 font-bold mt-1">กดปุ่ม &quot;ยืนยันการออกเดินทาง&quot; เมื่อเริ่มล้อหมุน</p>
      </div>
    </div>
  );
};
